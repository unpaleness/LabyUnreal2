...
void ALabyActor::GenerateMaze() {
	...
	while (N > 0) {
		if (++it > MaxIterations) {
			////UE_LOG(LogLabyActor, Warning, TEXT("%s: Limit of %i iteration exceeded!"), *GetName(), MaxIterations);
			break;
		}
		//UE_LOG(LogLabyActor, Display, TEXT("%s: N: %i, Processing (%i, %i)"), *GetName(), N, Path[N - 1] % X, Path[N - 1] / X);
		int32 NextCell{ GetRandomNeighbour(Path[N - 1], Cells) };
		if (NextCell >= X * Y) {
			UE_LOG(LogLabyActor, Warning, TEXT("%s: Next cell index (%I) is above maximum possible (%i)!"), *GetName(), NextCell, X * Y - 1);
			break;
		}
		if (NextCell >= 0) {
			Path[N] = NextCell;
...
int32 ALabyActor::GetRandomNeighbour(int32 Index, bool* Cells) {
	...
	//UE_LOG(LogLabyActor, Display, TEXT("%s: Neighbours %i: %i, %i, %i, %i"), *GetName(), Dirs, AvailDirs[0], AvailDirs[1], AvailDirs[2], AvailDirs[3]);
	// No neighbours
	if (Dirs <= 0) {
		return -1;
	}
	...
	for (uint8_t i = 0; i < 4; ++i) {
		...
		if (ValidDir == Dir) {
			switch (i) {
			case 0:
				//UE_LOG(LogLabyActor, Display, TEXT("%s: Southern dir (%i, %i)"), *GetName(), (Index - X) % X, (Index - X) / X);
				return Index - X;
			case 1:
				//UE_LOG(LogLabyActor, Display, TEXT("%s: Northern dir (%i, %i)"), *GetName(), (Index + X) % X, (Index + X) / X);
				return Index + X;
			case 2:
				//UE_LOG(LogLabyActor, Display, TEXT("%s: Eastern dir (%i, %i)"), *GetName(), (Index - 1) % X, (Index - 1) / X);
				return Index - 1;
			case 3:
				//UE_LOG(LogLabyActor, Display, TEXT("%s: Western dir (%i, %i)"), *GetName(), (Index + 1) % X, (Index + 1) / X);
				return Index + 1;
			}
			break;
		}
...
void ALabyActor::BreakWall(int32 C1, int32 C2) {
	//UE_LOG(LogLabyActor, Display, TEXT("%s: Breaking wall (%i,%i)|(%i,%i)"), *GetName(), C1 % X, C1 / X, C2 % X, C2 / X);
	...
	UE_LOG(LogLabyActor, Warning, TEXT("%s: Breaking wall (%i,%i)|(%i,%i) failed!"), *GetName(), C1 % X, C1 / X, C2 % X, C2 / X);
}
